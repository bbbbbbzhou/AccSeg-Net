...
def define_S(input_nc, output_nc, ngf, netS, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netS == 'resnet_9blocks':
        ...
        net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6, gpu_ids=gpu_ids)
    elif netS == 'unet':
        net = UnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True, use_duse=False).cuda()  # Naive U-Net
    elif netS == 'duseunet':
        net = UnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True, use_duse=True).cuda()   # UNet with Spatial-Channel SE
    elif netS == 'r2unet':
        net = R2UnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True).cuda()   # Dense U-Net
    elif netS == 'attunet':
        net = AttentionUnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True).cuda()   # Attention U-Net
    elif netS == 'addunet':
        net = ADDUnetGenerator_SEG(input_nc, output_nc, ngf, bilinear=True, use_duse=True).cuda()   # Attention Dense U-Net with scSE
    else:
...
'''
UNet with or without spatial-channel squeeze and excitation block
'''
...
class UnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, bilinear=False, use_duse=False):
        ...
        self.inc = DoubleConv(self.n_channels, ngf, use_duse=use_duse)
        self.down1 = Down(ngf, 2*ngf, use_duse=use_duse)
        self.down2 = Down(2*ngf, 4*ngf, use_duse=use_duse)
        self.down3 = Down(4*ngf, 8*ngf, use_duse=use_duse)
        self.down4 = Down(8*ngf, 8*ngf, use_duse=False)
        self.up1 = Up(16*ngf, 4*ngf, bilinear, use_duse=use_duse)
        self.up2 = Up(8*ngf, 2*ngf, bilinear, use_duse=use_duse)
        self.up3 = Up(4*ngf, ngf, bilinear, use_duse=use_duse)
...
R2U-Net
'''
class R2UnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, t=2, bilinear=False):
        super(R2UnetGenerator_SEG, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.Upsample = nn.Upsample(scale_factor=2)

        self.RRCNN1 = RRCNN_block(ch_in=input_nc, ch_out=ngf, t=t)

        self.RRCNN2 = RRCNN_block(ch_in=ngf, ch_out=2*ngf, t=t)

        self.RRCNN3 = RRCNN_block(ch_in=2*ngf, ch_out=4*ngf, t=t)

        self.RRCNN4 = RRCNN_block(ch_in=4*ngf, ch_out=8*ngf, t=t)

        self.RRCNN5 = RRCNN_block(ch_in=8*ngf, ch_out=16*ngf, t=t)

        self.Up5 = up_conv(ch_in=16*ngf, ch_out=8*ngf)
        self.Up_RRCNN5 = RRCNN_block(ch_in=16*ngf, ch_out=8*ngf, t=t)

        self.Up4 = up_conv(ch_in=8*ngf, ch_out=4*ngf)
        self.Up_RRCNN4 = RRCNN_block(ch_in=8*ngf, ch_out=4*ngf, t=t)

        self.Up3 = up_conv(ch_in=4*ngf, ch_out=2*ngf)
        self.Up_RRCNN3 = RRCNN_block(ch_in=4*ngf, ch_out=2*ngf, t=t)

        self.Up2 = up_conv(ch_in=2*ngf, ch_out=ngf)
        self.Up_RRCNN2 = RRCNN_block(ch_in=2*ngf, ch_out=ngf, t=t)

        self.Conv_1x1 = nn.Conv2d(ngf, output_nc, kernel_size=1, stride=1, padding=0)
...
Attention U-Net
'''
class AttentionUnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, bilinear=False):
        super(AttentionUnetGenerator_SEG, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.Conv1 = conv_block(ch_in=input_nc, ch_out=ngf)
        self.Conv2 = conv_block(ch_in=ngf, ch_out=2*ngf)
        self.Conv3 = conv_block(ch_in=2*ngf, ch_out=4*ngf)
        self.Conv4 = conv_block(ch_in=4*ngf, ch_out=8*ngf)
        self.Conv5 = conv_block(ch_in=8*ngf, ch_out=16*ngf)

        self.Up5 = up_conv(ch_in=16*ngf, ch_out=8*ngf)
        self.Att5 = Attention_block(F_g=8*ngf, F_l=8*ngf, F_int=4*ngf)
        self.Up_conv5 = conv_block(ch_in=16*ngf, ch_out=8*ngf)

        self.Up4 = up_conv(ch_in=8*ngf, ch_out=4*ngf)
        self.Att4 = Attention_block(F_g=4*ngf, F_l=4*ngf, F_int=2*ngf)
        self.Up_conv4 = conv_block(ch_in=8*ngf, ch_out=4*ngf)

        self.Up3 = up_conv(ch_in=4*ngf, ch_out=2*ngf)
        self.Att3 = Attention_block(F_g=2*ngf, F_l=2*ngf, F_int=ngf)
        self.Up_conv3 = conv_block(ch_in=4*ngf, ch_out=2*ngf)

        self.Up2 = up_conv(ch_in=2*ngf, ch_out=ngf)
        self.Att2 = Attention_block(F_g=ngf, F_l=ngf, F_int=int(ngf/2))
        self.Up_conv2 = conv_block(ch_in=2*ngf, ch_out=ngf)
...
'''
Attention Dense U-Net with scSE block
'''
class ADDUnetGenerator_SEG(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, bilinear=False, use_duse=False):
        super(ADDUnetGenerator_SEG, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.Conv1 = dense_conv_block(ch_in=input_nc, ch_out=ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)
        self.Conv2 = dense_conv_block(ch_in=ngf, ch_out=2*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)
        self.Conv3 = dense_conv_block(ch_in=2*ngf, ch_out=4*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)
        self.Conv4 = dense_conv_block(ch_in=4*ngf, ch_out=8*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)
        self.Conv5 = dense_conv_block(ch_in=8*ngf, ch_out=16*ngf, nDenselayer=5, growthRate=32, use_duse=False)

        self.Up5 = up_conv(ch_in=16*ngf, ch_out=8*ngf)
        self.Att5 = Attention_block(F_g=8*ngf, F_l=8*ngf, F_int=4*ngf)
        self.Up_conv5 = dense_conv_block(ch_in=16*ngf, ch_out=8*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)

        self.Up4 = up_conv(ch_in=8*ngf, ch_out=4*ngf)
        self.Att4 = Attention_block(F_g=4*ngf, F_l=4*ngf, F_int=2*ngf)
        self.Up_conv4 = dense_conv_block(ch_in=8*ngf, ch_out=4*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)

        self.Up3 = up_conv(ch_in=4*ngf, ch_out=2*ngf)
        self.Att3 = Attention_block(F_g=2*ngf, F_l=2*ngf, F_int=ngf)
        self.Up_conv3 = dense_conv_block(ch_in=4*ngf, ch_out=2*ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)

        self.Up2 = up_conv(ch_in=2*ngf, ch_out=ngf)
        self.Att2 = Attention_block(F_g=ngf, F_l=ngf, F_int=int(ngf/2))
        self.Up_conv2 = dense_conv_block(ch_in=2*ngf, ch_out=ngf, nDenselayer=5, growthRate=32, use_duse=use_duse)
...
class dense_conv_block(nn.Module):
    def __init__(self, ch_in, ch_out, nDenselayer, growthRate, use_duse):
        ...
        self.conv_1x1 = nn.Conv2d(nChannels_, ch_out, kernel_size=1, padding=0, bias=False)
        self.bn = nn.BatchNorm2d(ch_out)

        self.use_duse = use_duse
        if self.use_duse:
            self.scSE = ChannelSpatialSELayer(ch_out)
